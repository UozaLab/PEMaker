...
namespace ckfilesystem
{
    ...
    bool ElTorito::add_boot_image_no_emu(const ckcore::tchar *full_path,const ckcore::tchar *internal_path,bool bootable,
                                         ckcore::tuint16 load_segment,ckcore::tuint16 sec_count)
    {
        if (boot_images_.size() >= ELTORITO_MAX_BOOTIMAGE_COUNT)
            return false;
        ...
        boot_images_.push_back(new ElToritoImage(
            full_path,internal_path,bootable,ElToritoImage::EMULATION_NONE,load_segment,sec_count));
    ...
    ckcore::tuint64 ElTorito::get_boot_data_size()
    {
        ...
        for (it = boot_images_.begin(); it != boot_images_.end(); it++)
            ...
            size += (ckcore::tuint64)ckcore::File::size((*it)->full_path_.c_str());
