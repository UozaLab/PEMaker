    ...
    */
    ...
    void IsoWriter::write_header(const FileSet &files,FileTree &file_tree)
    {
        ...
        if (file_sys_.eltorito_.get_boot_image_count() > 0)
        {
            ...
            {
                for(int i=0; i<file_sys_.eltorito_.get_boot_image_count(); i++)
                {
                    FileTreeNode* node = file_tree.get_node_from_path(file_sys_.eltorito_.get_image(i)->internal_path_.c_str());
                    if(node != NULL)
                    {
                        file_sys_.eltorito_.get_image(i)->data_sec_pos_ = (ckcore::tuint32) node->data_pos_normal_;
                    }
                }
            }
        }
        ...
        if (file_sys_.eltorito_.get_boot_image_count() > 0)
        {
            ...
            {
                FileTreeNode* node = file_tree.get_node_from_path(catalog_internal_path.c_str());
                if(node != NULL)
                {
                    ckcore::tuint32 boot_cat_sec = (ckcore::tuint32) node->data_pos_normal_;
                    file_sys_.eltorito_.write_boot_record(out_stream_, boot_cat_sec);
                }
            }
        ...
        else
        {
            ...
            node->file_stream_.close();
            ckcore::FileOutStream filestream(catalog_file_path.c_str());
            if (!filestream.open())
            {
                node->file_stream_.open();
                return;
            }
            SectorOutStream sos(filestream);
            file_sys_.eltorito_.write_boot_catalog(sos);
            filestream.close();
            node->file_stream_.open();
        }
    }
