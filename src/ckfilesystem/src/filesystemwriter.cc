...
namespace ckfilesystem
{
    FileSystemWriter::FileSystemWriter(ckcore::Log &log,FileSystem &file_sys,
                                       bool fail_on_error) :
        log_(log),file_sys_(file_sys),file_tree_(log),fail_on_error_(fail_on_error),extended_mode(false)
    {
    }
                                ...
                                ckcore::tuint32 sec_offset)
    {
        ...
        try
        {
            ...
            if(extended_mode)
            {
                iso_writer.set_catalog_file_path(catalog_file_path.c_str(), catalog_internal_path.c_str());
            }
