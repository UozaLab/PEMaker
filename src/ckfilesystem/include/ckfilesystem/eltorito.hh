...
namespace ckfilesystem
{
    ...
    public:
        ...
        ElToritoImage(const ckcore::tchar *full_path,const ckcore::tchar *internal_path,bool bootable,Emulation emulation,
                      ckcore::tuint16 load_segment,ckcore::tuint16 sec_count) : 
            full_path_(full_path),internal_path_(internal_path),extended_mode(true),bootable_(bootable),emulation_(emulation),
        ...
        ElToritoImage(const ckcore::tchar *full_path,bool bootable,Emulation emulation,
                      ckcore::tuint16 load_segment,ckcore::tuint16 sec_count) : 
            full_path_(full_path),internal_path_(ckT("")),extended_mode(false),bootable_(bootable),emulation_(emulation),
            load_segment_(load_segment),sec_count_(sec_count),data_sec_pos_(0)
        {
        }
    };
    ...
    public:
        ...
        bool add_boot_image_no_emu(const ckcore::tchar *full_path,bool bootable,
                                   ckcore::tuint16 load_segment,ckcore::tuint16 sec_count);
        bool add_boot_image_no_emu(const ckcore::tchar *full_path,const ckcore::tchar *internal_path,bool bootable,
                                   ckcore::tuint16 load_segment,ckcore::tuint16 sec_count);
		...
		ElToritoImage* get_image(int index){ return boot_images_[index];}
    };
};
